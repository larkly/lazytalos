...
func (m Model) switchTab(idx int) (Model, tea.Cmd) {
	...
	case "network":
		...
		if !m.tabInited[idx] {
			m.network = network.New(m.client, m.refreshInterval)
	...
	case "storage":
		...
		if !m.tabInited[idx] {
			m.storage = storage.New(m.client, m.refreshInterval)
	...
	case "etcd":
		...
		if !m.tabInited[idx] {
			m.etcdView = etcdview.New(m.client, m.refreshInterval)
			m.etcdView.SetSize(m.width, m.contentHeight())
			m.tabInited[idx] = true
			m.statusBar.Hint = m.etcdView.Hints()
			return m, m.etcdView.Init()
		}
		m.statusBar.Hint = m.etcdView.Hints()
